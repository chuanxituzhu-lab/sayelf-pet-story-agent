...
- 用户提交的文字、单张或最多两张参考图片会通过本地 WebUI 后端生成视频，并回传到当前页面播放和下载；默认每天免费 1 支视频和 2 张输出图片，超出后按展位积分与后台选择的模型单价扣费。
- 后台二维码控制台可为展位选择模型、增加本地积分，并录入参展商 Logo 或展板；品牌素材按原始比例 contain 后植入，不做非等比拉伸。
...
- 额度按本机日期计算：每天免费 1 支视频、2 张输出图片；额外数量按照展位积分和模型目录单价扣减。失败渲染会释放本次额度和积分，成功结果可通过返回的播放地址、下载地址和图片地址读取。
- 后台选择的模型绑定到展位二维码；当前 `local-fast` 与 `local-cinematic` 都是本地演示模型，真实大模型 provider 仍需在同一 adapter 边界接入。
- Logo/展板文件只保存在 `webui/.local/brand_assets/`，在视频帧和输出图片中按原始宽高比缩放；源图不被拉伸，成片采用高质量本地编码。
